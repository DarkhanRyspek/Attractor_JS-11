...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 exclude-result-prefixes="xsd ppp html">
   ...
   <!-- Set the base URL for image resolving. -->
	 <xsl:param name="base_IMG_URL"/>

	 <!-- If 'true', prints out diagram images. -->
	 <xsl:param name="printDiagram">true</xsl:param>
   ...
   <xsl:param name="DEFAULT_DOCUMENT_EXT">.htm</xsl:param>
   ...
   <!-- External Modules -->
	 <xsl:include href="xs3p_diagram_module.xsl"/>
...
.footer {
   ...
   <xsl:template name="PrintAnnotation">
      ...
      <!-- Diagram -->
		  <xsl:if test="normalize-space(translate($printDiagram, 'TRUE', 'true'))='true'">
			  <xsl:call-template name="PrintDiagram">
				  <xsl:with-param name="base_IMG_URL" select="$base_IMG_URL"/>
			  </xsl:call-template>
		  </xsl:if>
...
'documentation' elements within its 'annotation' element.
   ...
   <xsl:template name="PrintSampleSimpleRestriction">
      ...
      <xsl:if test="$restriction/xsd:enumeration">
         <xsl:text> (</xsl:text>
         <xsl:call-template name="PrintEnumFacetsLine">
            <xsl:with-param name="simpleRestrict" select="$restriction"/>
         </xsl:call-template>
         <xsl:text>)</xsl:text>
      </xsl:if>
   ...
   <xsl:template name="PrintCompName">
      ...
      <xsl:variable name="baseURI">
         <xsl:choose>
            ...
            <xsl:when test="normalize-space($location)!='this'">
               <xsl:call-template name="GetSchemaDocLocation">
                  <xsl:with-param name="uri" select="$location"/>
               </xsl:call-template>
            </xsl:when>
         </xsl:choose>
      </xsl:variable>
      ...
      <xsl:variable name="title">
         <xsl:choose>
            ...
            <xsl:when test="$errMsg!=''">
               <xsl:text>"</xsl:text><xsl:value-of select="$localName"/><xsl:text>" </xsl:text>
      ...
      <xsl:variable name="link">
         <xsl:choose>
            ...
            <xsl:otherwise>
               ...
               <!-- Anchor within URI -->
               <xsl:value-of select="concat('#',normalize-space($compPrefix),normalize-space($localName))"/>
            </xsl:otherwise>
         </xsl:choose>
      </xsl:variable>
...
Unknown schema component, <xsl:value-of select="local-name($component)"/>.
   ...
   <xsl:template name="GetThisPrefix">
      <xsl:if test="/xsd:schema/@targetNamespace">
         <xsl:variable name="prefix" select="local-name(/xsd:schema/namespace::*[normalize-space(.)=normalize-space(/xsd:schema/@targetNamespace)])"/>
         <xsl:if test="not($prefix = 'xmlns')">
            <xsl:value-of select="$prefix"/>
         </xsl:if>
      </xsl:if>
   </xsl:template>
...
was not specified in the links file, <xsl:value-of select="$linksFile"/>.
      ...
      <xsl:otherwise>
        <xsl:value-of select="concat($uri, $DEFAULT_DOCUMENT_EXT)"/>
      </xsl:otherwise>
      </xsl:choose>
   </xsl:template>
   ...
   <!--
     ...
     Param(s):
            simpleRestrict (Node) required
              'restriction' element
     -->
   <xsl:template name="PrintEnumFacetsLine">
      <xsl:param name="simpleRestrict"/>
